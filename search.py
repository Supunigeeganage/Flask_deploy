...
def load_data():
    ...
    with open("data.csv") as f:
        ...
        for row in reader:
            if (row["title"], row["performer"]) not in song_data:
                song_data[(row["title"], row["performer"])] = {
                    "chart_debut": row["chart_debut"],
                    "peak_position": int(row["peak_position"]),
                    "time_on_chart": int(row["time_on_chart"])
                }
            else:
                song_data[(row["title"], row["performer"])]["peak_position"] = min(song_data[(row["title"], row["performer"])]["peak_position"], int(row["peak_position"]))
                song_data[(row["title"], row["performer"])]["time_on_chart"] = max(song_data[(row["title"], row["performer"])]["time_on_chart"], int(row["time_on_chart"]))
